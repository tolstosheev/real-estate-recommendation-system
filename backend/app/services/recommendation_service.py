...
class RecommendationService:
    ...
    async def _build_user_vector(self, user_id: str) -> Optional[np.ndarray]:
        ...
        if prefs:
            ...
            area = (float(prefs.min_area or 0) + 100) / \
                2 if prefs.min_area else 50.0
            rooms = sum(prefs.preferred_rooms) / \
                len(prefs.preferred_rooms) if prefs.preferred_rooms else 2.0
        ...
        user_vec = self.utils.compute_user_profile_vector(
            explicit_vec, implicit_vec)
    ...
    async def recommend(self, user_id: str, limit: int = 10) -> List[Property]:
        ...
        prop_vectors = np.array([self._get_property_vector(p)
                                for p in properties])
        ...
        norm_prop_vectors, scaler = self.utils.normalize_features(
            prop_vectors.tolist())
        ...
        for i, prop in enumerate(properties):
            ...
            score = self.utils.calculate_cosine_similarity(
                norm_user_vec, p_vec)
