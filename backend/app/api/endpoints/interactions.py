...
@router.post("/",
             ...
             description="Like or view a property")
    ...
    for prop in favorites:
        ...
        if prop_data:
            ...
            if isinstance(prop_data, dict):
                ...
                # It's a SQLAlchemy object - convert to dict
                owner_data = None
...
@router.get("/history", response_model=List[PropertyOut],
               summary="Get View History",
               description="List viewed properties")
async def get_my_history(
    ...
    for prop in history:
        ...
        if prop_data:
            # If it's already a dict (from cache), use it directly
            if isinstance(prop_data, dict):
                liked = await service.interaction_repo.find_interaction(str(current_user.id), str(prop.id), interaction_type='like')
