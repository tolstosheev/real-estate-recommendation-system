...
class PropertyRepository:
    ...
    async def get_all(self, limit: int = 100, offset: int = 0, 
        ...
        if lat is not None and lon is not None and radius_km is not None:
            ...
            query = query.filter(func.ST_DWithin(cast(Property.location, Geography), cast(point, Geography), radius_km * 1000))
    ...
    async def update(self, property_id: str, update_data: dict) -> Property | None:
        ...
        query = update(Property).where(Property.id == property_id).values(**update_data).execution_options(synchronize_session="fetch")
